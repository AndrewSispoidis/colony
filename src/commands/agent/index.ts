...
export async function agentInitCommand(): Promise<void> {
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      ...
      name: "description",
      message: "Description:",
      validate: (input: string) => (input.length > 0 ? true : "Description is required"),
    },
    {
      type: "input",
      name: "author",
...
export async function agentTestCommand(options: {
  ...
  const spec = yaml.load(raw) as { name: string; system_prompt: string; [key: string]: unknown };
  ...
  console.log(chalk.green(`\n✅ Test passed`));
  console.log(
    chalk.dim(`📊 ${result.tokens_used} tokens, ${(result.duration_ms / 1000).toFixed(1)}s`),
  );
}
