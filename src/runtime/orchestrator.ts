...
import type { WorkflowSpec, WorkflowResult, AgentResult, AgentSpec } from "../types/index.js";
...
async function runParallel(
  ...
  const agentsAndSteps: { agent: AgentSpec; input: string; index: number }[] = [];
  for (let i = 0; i < workflow.steps.length; i++) {
    const step = workflow.steps[i];
    const agent = resolveAgent(step.agent);
    if (!agent) {
  ...
  const spinners = parallelAgents.map(a =>
    ora({ text: `  ${a.agent.name}...`, color: "cyan" }).start()
  );
...
async function runCollaborative(
  workflow: WorkflowSpec,
  verbose: boolean,
): Promise<WorkflowResult> {
  const memory = createWorkingMemory();
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    for (let i = 0; i < agents.length; i++) {
      ...
      const issues = memory.entries.filter(e =>
        e.key.startsWith("issue_") && e.value.toLowerCase().includes(agent.name.toLowerCase())
      );
      ...
      if (prevOutput) {
        ...
        if (similarity < 0.9) allConverged = false;
      } else {
        allConverged = false;
      }
