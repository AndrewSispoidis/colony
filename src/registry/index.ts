...
const BUNDLED_AGENTS: Record<string, AgentSpec> = {
...
export function resolveAgent(nameOrPath: string): AgentSpec | null {
  if (nameOrPath.startsWith("./") || nameOrPath.startsWith("/")) {
    ...
    if (fs.existsSync(yamlPath)) {
      ...
      return yaml.load(raw) as AgentSpec;
    }
...
export function getInstalledAgentPath(name: string): string | null {
  const agentsDir = getAgentsDir();
  const agentDir = path.join(agentsDir, name.replace("/", path.sep));
...
export function getBundledAgentNames(): string[] {
