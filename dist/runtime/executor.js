...
async function scanWithCrawdad(content) {
    try {
        ...
        if (err instanceof Error && err.message.startsWith("Crawdad blocked"))
            throw err;
    }
}
export async function checkCrawdadStatus() {
    try {
...
async function executeWithAnthropic(agent, userMessage, verbose) {
...
async function executeWithOpenAI(agent, userMessage, verbose) {
...
export async function executeAgent(agent, input, context, verbose = false) {
